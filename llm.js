...
async function getFormText(event) {
  ...
  try {
    ...
    // Display the generated code and analysis
    ...
    const context = {
      ...
      chart_name: parsedResult.graph_type,
      x_ax_name: data_type === "Temperature" ? "Temperature (°C)" : "Precipitation (in)",
      y_ax_name: "Frequency"
    };

    // Wrap the generated code in a try-catch block
    const wrappedCode = `
      try {
        ${parsedResult.code}
      } catch (error) {
        console.error('Error in generated code:', error);
        throw new Error('Error in generated code: ' + error.message);
      }
    `;

    // Execute the generated code within the prepared context
    const executeCode = new Function(...Object.keys(context), wrappedCode);
    executeCode(...Object.values(context));
...
function parseCSV(csvString) {
  ...
  const parsedData = d3.csvParse(commaCSV, d => {
    return {
      ...
      y: parseFloat(d[Object.keys(d)[1]]) // Second column is temperature or precipitation
    };
  });

  // Filter out any invalid entries and create a clean array of objects
